...
from .core.actor import Actor, ActorNotFound
from .core.exceptions import AuthPermsDataError, BaseArgumentsError
...
class AuthPermsFlask(Flask):
    ...
    def add_url_rule(self, rule, endpoint=None, view_func=None, **options):
        ...
        """
        return (
            super(AuthPermsFlask, self).add_url_rule(
                rule, endpoint, view_func, **options
            )
            if not endpoint in self.config.get("SKIP_SUBMODULE_ENDPOINTS", [])
            else None
        )
...
class AuthPerms:
    ...
    def validate_base_args(self):
        ...
        if not self.base_args.get("SERVICE_PRIVATE_KEY"):
            raise AuthPermsDataError(
                "SERVICE_PRIVATE_KEY is required parameter for configuring auth permission part. "
        ...
        if not self.base_args.get("AUTH_STANDALONE"):
            if not self.base_args.get("AUTH_PUB_KEY"):
                raise AuthPermsDataError(
                    "AUTH_PUB_KEY is required parameter for configuring auth permission part. "
                    "\n AUTH_PUB_KEY - auth public key where your service registered."
                )

        if not self.base_args.get("AUTH_STANDALONE"):
            query = """SELECT EXISTS(SELECT 1 FROM actor WHERE initial_key=%s)"""
            if not self.app.db.fetchone(
                query, [self.base_args.get("AUTH_PUB_KEY")]
            ).get("exists"):
                raise AuthPermsDataError(
                    "Auth service is not registered in your database. \n Please register it."
                )
        ...
        if not self.base_args.get("SERVICE_UUID"):
            ...
            if not service_uuid:
                raise AuthPermsDataError(
                    "SERVICE_UUID or SERVICE_PUBLIC_KEY is required parameter for configuring "
        ...
        if not self.app.db.fetchone(query, [self.base_args.get("SERVICE_UUID")]).get(
            "exists"
        ):
            raise AuthPermsDataError(
                "Your service is not registered in your database. \n Please register it."
            )
        ...
        if not self.app.db.fetchone(query).get("exists"):
            raise AuthPermsDataError(
                "There is no DEFAULT group in your database. \n Please create it based on auth "
                "information."
            )
        ...
        if not self.app.db.fetchone(query).get("exists"):
            raise AuthPermsDataError(
                "There is no ADMIN group in your database. \n Please create it based on auth "
                "information."
            )
        ...
        if not self.app.db.fetchone(query).get("exists"):
            raise AuthPermsDataError(
                "There is no BAN group in your database. \n Please create it based on auth "
                "information."
            )

        if not self.base_args.get("PRIMARY_KEY_ONLY"):
            print(
                "NOTICE! You have not set PRIMARY_KEY_ONLY option. This option is used for signature verification "
                "on authentication with only first time generated keys pair."
            )

        if not self.base_args.get("DEFAULT_GROUP_NAME"):
            print(
                "NOTICE! You have not set DEFAULT_GROUP_NAME option. This option is used for automatically adding "
        ...
        if not self.base_args.get("SESSION_STORAGE"):
            print(
                "NOTICE! You have not set SESSION_STORAGE option. This option is used for adding default js "
        ...
        if not self.base_args.get("DEPENDED_SERVICES"):
            print(
                "NOTICE! You have not pass DEPENDED_SERVICES variable. This variable is used to set depended services. "
        ...
        if not self.base_args.get("REDIRECT_URL"):
            print(
                "NOTICE! You have not pass REDIRECT_URL variable."
                "This variable is used to set first page after login, if you request login doesn't have referrer. "
            )
            self.base_args["REDIRECT_URL"] = "/"

        if not self.base_args.get("BABEL_DEFAULT_LOCALE"):
            print(
                "NOTICE! You have not pass BABEL_DEFAULT_LOCALE variable. This variable is used to set default "
            ...
            if self.base_args.get("LANGUAGES"):
                for code in self.base_args.get("LANGUAGES"):
                    if code == "ru":
                        ...
                        print(
                            "Unknown language code in LANGUAGES variable, please check it - %s. "
                            "Add language information in list LANGUAGES_INFORMATION in settings file like\n "
                            'LANGUAGES_INFORMATION=[{"code": "en", "name": "English"}, ...]'
                            % code
                        )
                        raise BaseArgumentsError(
                            message="LANGUAGE_INFORMATION error. Unknown code."
                        )

                    self.base_args["LANGUAGE_INFORMATION"].append(
                        {"code": code, "name": name}
                    )
        else:
        ...
        if not self.base_args.get("DB_MINIMUM_CONNECTIONS"):
            ...
            if not isinstance(self.base_args.get("DB_MINIMUM_CONNECTIONS"), int):
                try:
                    self.base_args["DB_MINIMUM_CONNECTIONS"] = int(
                        self.base_args.get("DB_MINIMUM_CONNECTIONS")
                    )
        ...
        if not self.base_args.get("DB_MAXIMUM_CONNECTIONS"):
            ...
            if not isinstance(self.base_args.get("DB_MAXIMUM_CONNECTIONS"), int):
                try:
                    self.base_args["DB_MAXIMUM_CONNECTIONS"] = int(
                        self.base_args.get("DB_MAXIMUM_CONNECTIONS")
                    )
                except Exception as e:
                    self.base_args["DB_MAXIMUM_CONNECTIONS"] = 10
    ...
    def set_base_args(self, app):
        ...
        """
        if not self.base_args.get(
            "BABEL_TRANSLATION_DIRECTORIES"
        ) or LOCALIZATION_PATH not in self.base_args.get(
            "BABEL_TRANSLATION_DIRECTORIES"
        ):
            if not self.base_args.get("BABEL_TRANSLATION_DIRECTORIES"):
                self.base_args["BABEL_TRANSLATION_DIRECTORIES"] = (
                    LOCALIZATION_PATH + ";"
                )
            else:
                self.base_args["BABEL_TRANSLATION_DIRECTORIES"] += (
                    LOCALIZATION_PATH
                    if self.base_args["BABEL_TRANSLATION_DIRECTORIES"].endswith(";")
                    else ";" + LOCALIZATION_PATH + ";"
                )

        if not self.base_args.get("SERVICE_UUID") and self.table_exists(
            table_name="actor"
        ):
    ...
    def configure_db(self, app):
        ...
        """
        min_connections = app.config.get(
            "DB_MINIMUM_CONNECTIONS", self.base_args.get("DB_MINIMUM_CONNECTIONS", 1)
        )
        max_connections = app.config.get(
            "DB_MAXIMUM_CONNECTIONS", self.base_args.get("DB_MAXIMUM_CONNECTIONS", 10)
        )
        if self.database_credentials:
            DatabaseManager(
                database=self.database_credentials,
                min_connection=min_connections,
                max_connections=max_connections,
            ).init_app(app=app)
        elif self.database_credentials_dsn:
            DatabaseManager(
                dsn=self.database_credentials_dsn,
                min_connection=min_connections,
                max_connections=max_connections,
            ).init_app(app=app)
        elif self.base_args.get("DATABASE"):
            DatabaseManager(
                database=self.base_args.get("DATABASE"),
                min_connection=min_connections,
                max_connections=max_connections,
            ).init_app(app=app)
        elif self.base_args.get("DATABASE_URI"):
            DatabaseManager(
                dsn=self.base_args.get("DATABASE_URI"),
                min_connection=min_connections,
                max_connections=max_connections,
            ).init_app(app=app)
        else:
            raise AuthPermsDataError(
                "There was no database information passed. You can pass in database_credentials "
