...
class PermissionView(MethodView, BasePermissionView):
    ...
    POST: /perms/
    @POST Create permissions in database that was sent from auth based on request body@
    @DELETE Delete permissions from database that was sent from auth based on request body@
    """
    ...
    def post(self):
        ...
        :return:
        @subm_flow Create permissions in database that was sent from auth
        """
        ...
        if not app.db.fetchone(
            ...
            try:
                app.db.execute(
                    """INSERT INTO actor SELECT * FROM jsonb_populate_record(null::actor, jsonb %s)""",
                    [json_dumps(actor)],
                )
            except Exception as e:
                print("Error with creating actor. Exception - %s" % e)
                response = create_response_message(
                    message=_("Some error occurred while creating actor."), error=True
                )
                return make_response(jsonify(response), 400)

        permission = data.get("permission")
        perm = app.db.fetchall(
            """SELECT * FROM insert_or_update_perms(%s)""", [json_dumps((permission))]
        )
        if not perm:
            response = create_response_message(
                message=_("Some error occurred while creating permissions."), error=True
            )
            return make_response(jsonify(response), 400)

        SendCallback(action_type="create_permission", data=permission).send_callback()
        response = dict(message=_("Permission successfully updated."))
    ...
    def delete(self):
        ...
        perm = app.db.fetchone(
            """DELETE FROM permissions WHERE actor_id=%s AND perm_id=%s RETURNING *""",
        ...
        if not perm:
            response = create_response_message(
                message=_(
                    "There is no permission such permission %(perm_id)s " "in database",
        ...
        response = dict(message=_("Permission successfully deleted."))
        return make_response(jsonify(response), 200)
