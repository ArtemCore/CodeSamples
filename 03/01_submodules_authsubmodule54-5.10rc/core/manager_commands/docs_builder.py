...
def build_docs_service(service_path, service_name):
    ...
    child_docs = DocsGenerator(
        service_path, service_name, data_files.get_dict_order_by_path(service_path)
    )
...
def generate_file_tree(path):
    ...
    if file_name not in ignore_files:
        if (
            file_name.rsplit(".", 1)[-1] == "py"
            or file_name.rsplit(".", 1)[-1] == file_name
        ):
            d = {"name": file_name}
            ...
            if os.path.isdir(path):
                d["type"] = "directory"
                list_files = [
                    generate_file_tree(os.path.join(path, x)) for x in os.listdir(path)
                ]
    ...
    def set_doc_module(self, module_data):
                    ...
                    for method_name, method_data in class_data["methods"].items():
                        ...
                        if method_data is not None:
                            method_arguments = ", ".join(
                                item
                                for item in method_data["Arguments"]
                                if item != "self"
                            )
                            ...
                            self.modules.append(
                                "    **{}**({}):".format(
                                    method_data["name"], method_data["arguments"]
                                )
                            )
                            if method_data["description"]:
                                self.modules.append("    * ***Description***: <br>")
                                self.modules.append(
                                    "        {}".format(method_data["description"])
                                )
        ...
        if module_data["module_methods"]:
            ...
            for method_name, method_data in module_data["module_methods"].items():
                ...
                if method_data is not None:
                    method_arguments = ", ".join(
                        item for item in method_data["Arguments"] if item != "self"
                    )
                    self.modules.append("\n")
                    self.modules.append(
                        "*function* **{}**({}):".format(method_name, method_arguments)
                    )
...
class DocsGenerator:
    ...
    def generate_files(service_path, service_name, data):
        ...
        for catalog_key in data:
            ...
            save_file.set_tags()
            save_file.set_description(
                get_git_branch(service_path), service_name, catalog_key
            )
            ...
            save_file.path = (
                service_path + "/docs" + save_file.path.replace(service_path, "")
            )
...
class MainDocGenerator:
    ...
    def generate_file_data_from_tree(
        self, tree, service_path, tabs, node=0, fixed_node=None, last_file=None
    ):
        """
        ...
        if file_name != service_path.rsplit("/", 1)[-1]:
            ...
            if file_type == "directory":
                ...
                self.save_file.append(
                    "{}[***{}***]({}#module-{}) <br>".format(
                        print_tabs, file_name, doc_ref, file_name.replace(".", "")
                    )
                )
        ...
        if file_type == "directory":
            ...
            for x in file_nodes:
                ...
                self.generate_file_data_from_tree(
                    x, service_path, tabs, node + 1, fixed_node, last_file
                )
                tabs = [" "]
