...
class ServiceDocumentation(BuildDocumentationMixIn):
    ...
    def build_tree(self):
        ...
        if os.path.isdir(self.service_path):
            listdir = [
                file for file in os.listdir(self.service_path) if file not in ignore
            ]
            for file in listdir:
    ...
    def build_flows_tree(self):
        ...
        for flow_box in flow_boxes:
            ...
            for relation in relations:
                ...
                relation_obj = list(
                    filter(lambda x: x.id == item and x.number == number, flow_boxes)
                )
                ...
                try:
                    ...
                    raise SyntaxError(
                        f"Relation {relation} for flow box "
                        f"{flow_box.id}-{flow_box.number} {flow_box.description} don't exists"
                    )

        # Set flow boxes for flow_def
        for flow_def in flow_defs:
            flow_def.flow_boxes.extend(
                [flow_box for flow_box in flow_boxes if flow_box.id == flow_def.id]
            )
            flow_def.flow_boxes.sort(key=lambda x: x.number)

        for group in groups:
            group.flow_defs.extend(
                flow_def for flow_def in flow_defs if flow_def.id in group.id
            )
...
class Module(File):
    ...
    def get_groups_and_flows(self, docstring):
        ...
        while flow_box:
            ...
            self.flow_boxes.append(
                FlowBox(description, item, number, relations_str, decision)
            )
    ...
    def get_flow_boxes(self):
        with open(self.path) as module:
            ...
            comments = [
                token[1]
                for token in tokenize.generate_tokens(module.readline)
                if token[0] == 55
            ]
