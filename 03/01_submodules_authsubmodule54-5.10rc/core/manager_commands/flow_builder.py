...
def service_flows(service_path, service_name, filename):
    ...
    f_list_html_urls = generate_flows_list(
        service_path, flows_dict_list, groups_dict_list, True
    )
    f_list = generate_flows_list(service_path, flows_dict_list, groups_dict_list, False)

    flows_file_obj = FlowSaverPerformer(
        service_path, service_name, f_list_html_urls, filename
    )
            ...
            for step_obj in boxes:
                ...
                for submodule in get_submodules(service_path):
                    if step_obj.get("box_href").find(submodule.get("path")) >= 0:
                        href_submodule = submodule.get("https_url") + step_obj.get(
                            "box_href"
                        ).replace(
                            submodule.get("path"),
                            "blob/" + submodule.get("last_commit"),
                            1,
                        )
                        break
                ...
                if html_urls:
                    if step_obj.get("box_decision"):
                        ...
                        list_flows.append(
                            "{}[<a href='{}'>{}</a>]\n".format(
                                flow_obj["flow_id"] + step_obj["box_id"],
                                href,
                ...
                if step_obj.get("box_relations"):
                    ...
                    for s_rel in step_relations:
                        ...
                        if text_arrow:
                            ...
                            r_relations.append(
                                "{}-->{}\n".format(
                                    flow_obj["flow_id"] + step_obj["box_id"], s_rel_name
                                )
                            )
...
def create_dict_flows(flows, g_flows):
            ...
            try:
                flow_obj = next(
                    (
                        ...
                        if flow["flow_id"] == flow_id[0]
                        and flow["flow_path"] == flow_path
                    ),
                    False,
                )
            except (StopIteration, KeyError):
    ...
    for group in g_flows:
        ...
        group_path = group_path.group(1).replace(
            os.path.basename(group_path.group(1)), ""
        )
        ...
        if group_name:
            g_list.append(
                {"name": group_name.group(1), "path": group_path, "flows": group_list}
            )
...
class FlowScannerPerformer:
    ...
    def get_flows_list(self, path, service_path):
        ...
        with open(path) as file:
            ...
            class_definitions = [
                node for node in file_module.body if isinstance(node, ast.ClassDef)
            ]
            ...
            for class_def in class_definitions:
                ...
                (
                    flows_class_methods,
                    f_groups_class_methods,
                ) = self.get_flows_from_list_methods(
                    [
                        node
                ...
                if class_description:
                    ...
                    if f_class:
                        flows_list += [
                            f
                            + " %%{}#L{}%%".format(
                                path.replace(service_path, ""), str(class_def.lineno)
                            )
                            for f in f_class
                        ]
                    if g_class:
            ...
            try:
                ...
                if flows:
                    flows_list += [
                        f
                        + " %%{}#L{}%%".format(
                            path.replace(service_path, ""), str(current_module.lineno)
                        )
                        for f in flows
                    ]
                if f_groups:
    ...
    def get_flows_from_comments_in_file(path, service_path):
        flows_list = list()
        with open(path) as file:
            for toktype, tokval, begin, end, line in tokenize.generate_tokens(
                file.readline
            ):
                ...
                if match:
                    try:
                        ...
                        if flow_str:
                            ...
                            flow_str += " %%{}#L{}%%".format(
                                path.replace(service_path, "", 1), begin[0]
                            )
...
class FlowSaverPerformer:
    ...
    def __init__(self, service_path, service_name, flows_list, filename):
        self.saved_file = self.generate_file(
            service_path, service_name, flows_list, filename
        )
