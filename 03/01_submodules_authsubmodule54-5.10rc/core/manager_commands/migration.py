...
from ..migrations.base import backward_migration, forward_migration
...
class MigrateBase(Command):
    ...
    option_list = (Option("--name", "-n", dest="name"),)

    @staticmethod
    def __listdir_no_hidden(path: str) -> Generator[str, None, None]:
        """
    ...
    def _get_migrations_list(self) -> List[dict]:
        ...
        for path in migrations_paths:
            if path.endswith("auth_perms/core/migrations"):
                ...
                migrations_list += self._form_migration_dicts(raw_migrations_list, path)

        return main_migrations_list + migrations_list

    @staticmethod
    def _form_migration_dicts(
        migrations_list: List[str], folder_path: str
    ) -> List[dict]:
        """
        ...
        for name in migrations_list:
            migration = {"name": name, "path": f"{folder_path}.{name}"}
            result.append(migration)
        return result

    @staticmethod
    def _clean_and_sort(
        migrations_list: Union[Generator[str, None, None], list]
    ) -> List[str]:
        """
    ...
    def _get_migrations_paths() -> List[str]:
        ...
        for root, dirs, files in os.walk("."):
            if "migrations" in dirs:
                if root == ".":
                    migrations_paths.append(root.lstrip("./") + "migrations")
                else:
                    migrations_paths.append(root.lstrip("./") + "/migrations")
...
class MigrateForwards(MigrateBase):
    ...
    def apply_forward_migration(self, migration: dict) -> None:
        ...
        try:
            ...
            print("-" * 20, "ERROR", "-" * 20)
            print(
                "Migration can not be applied! Module import error - %s! Migration name - %s, migration path - %s"
        ...
        if not hasattr(mod, "Migration"):
            print("-" * 20, "ERROR", "-" * 20)
            print(
                "Migration has no class Migration! Migration name - %s, migration path - %s"
        ...
        if not inspect.isclass(mod.Migration):
            print("-" * 20, "ERROR", "-" * 20)
            print(
                "Migration is not a class! Migration name - %s, migration path - %s"
                % (migration.get("name", ""), migration.get("path", ""))
            )
            print("-" * 45)
            return
        ...
        if "BaseMigration" not in cls_parents:
            print("-" * 20, "ERROR", "-" * 20)
            print(
                "Migration has no parent BaseMigration! Migration name - %s, migration path - %s"
                % (migration.get("name", ""), migration.get("path", ""))
            )
            print("-" * 45)
            return
        ...
        if not hasattr(mod.Migration, "forwards_query"):
            print("-" * 20, "ERROR", "-" * 20)
            print(
                "Migration has no forwards_query! Migration name - %s, migration path - %s"
                % (migration.get("name", ""), migration.get("path", ""))
            )
            print("-" * 45)
            return

        forward_migration(mod.Migration(), mod.Migration.table_name, migration["name"])
...
class MigrateBackwards(MigrateBase):
    ...
    def run(self, name: str = None) -> None:
        ...
        if self.__verify_migration_existence(name):
            ...
            if name in migration_names:
                ...
                if ask in ("Y", "y", "yes", "Yes", "YES"):
                    for migration in further_migrations:
                        try:
                            chain_mod = self._get_migration_module(migration["path"])
                        except Exception as e:
                            print("-" * 20, "ERROR", "-" * 20)
                            print(
                        ...
                        if not hasattr(mod.Migration, "backwards_query"):
                            print("-" * 20, "ERROR", "-" * 20)
                            print(
                                "Migration has no backwards_query! Migration name - %s, migration path - %s"
                                % (migration.get("name", ""), migration.get("path", ""))
                            )
                            print("-" * 45)
                            return

                        backward_migration(chain_mod.Migration(), migration["name"])
                    backward_migration(mod.Migration(), name)
                    print("Successfully applied.")
                else:
                    print("Canceled.")
            else:
            ...
            print(
                f"Migration {name} does not mentioned in " f"migrations database table."
            )
    ...
    @staticmethod
    def __verify_migration_existence(file_name: str) -> bool:
        """
