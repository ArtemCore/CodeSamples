...
def get_path(exclude_directories):
    ...
    for root, dirs, files in os.walk("."):
        ...
        if root == ".":
            files_paths.append(root.lstrip("./"))
            for file in files:
                if file.endswith(".py"):
...
def compile(directory):
    ...
    if not directory:
        if app.config.get("BABEL_TRANSLATION_DIRECTORIES", None):
            directory = (
                app.config.get("BABEL_TRANSLATION_DIRECTORIES").split(";")[:-1]
                if app.config.get("BABEL_TRANSLATION_DIRECTORIES").endswith(";")
                else app.config.get("BABEL_TRANSLATION_DIRECTORIES").split(";")
            )
        else:
    ...
    try:
        ...
        print("#" * 15, "ERROR", "#" * 15)
        print(
            "Some error occurred while compiling translations in directory - %s! \n Exception - %s"
            % (directory, e)
        )
        print("#" * 35)
    return
...
def update(path, config, directory, exclude_directories):
    ...
    try:
        os.system(
            "pybabel extract -F %s -k _l -k _ -o messages.pot %s" % (config, path)
        )
    except Exception as e:
        print("#" * 15, "ERROR", "#" * 15)
        print(
            "Some error occurred while extracting translations in path - %s! \n Exception - %s"
            % (path, e)
        )
        print("#" * 35)
        return
...
def init(language, path, config, directory):
    ...
    try:
        ...
        print("#" * 15, "ERROR", "#" * 15)
        print(
            "Some error occurred while initialize directory - %s for language - %s! \n Exception - %s"
            % (path, language, e)
        )
        print("#" * 35)
