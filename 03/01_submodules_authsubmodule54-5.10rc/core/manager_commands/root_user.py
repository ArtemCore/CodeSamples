...
from ..ecdsa_lib import generate_key_pair, sign_data
from ..utils import get_default_user_group, hash_md5, json_dumps
...
class CreateRootUser(Command):
    ...
    def run(self, first_name=None, last_name=None, email=None, password=None):
        ...
        if not self.__check_table_exists():
            print(
                "\033[93mBefore creating root user, you should apply migrations.\033[0m"
            )
            return
        ...
        uinfo = {
            "email": email,
            "groups": [get_default_user_group().get("uuid")],
        ...
        root_signature = sign_data(
            app.config["SERVICE_PRIVATE_KEY"], root_uuid.__str__() + public_key
        )
        ...
        print(
            f'\033[92mRoot user {actor_uuid.get("uuid")} successfully created.\033[0m'
        )
        return
