...
class ConnectBiom(Command):
    ...
    def run(self, *args, **kwargs):
        ...
        if app.config.get("AUTH_STANDALONE") is True:
            print(
                "\033[91mYou can't use this command with AUTH_STANDALONE = True\033[0m"
            )
            return
        ...
        if not auth_uuid or not biom_name or not auth_pub_key or not auth_domain:
            print(
                f"\033[93mNot all auth credentials in local_settings.py\n"
        ...
        if (
            not service_uuid
            or not service_public_key
            or not service_domain
            or not service_name
        ):
            print(
                f"\033[93mNot all your service credentials on local_settings.py\n"
        ...
        status = subprocess.call(
            "pg_dump -a -t actor -t invite_link -t invite_link_temp -t migrations -t permissions"
            " -t phantom_relation -t salt_temp -t service_session_token -t temporary_session"
            " postgresql://{USER}:{PASSWORD}@{HOST}:{PORT}/{NAME} > ~/{0}_standalone_dump.sql".format(
                service_name, **db
            ),
            shell=True,
        )
        ...
        biom = {
            "initial_key": auth_pub_key,
            "actor_type": "service",
            "uinfo": {
                "service_name": "auth",
                "service_domain": auth_domain,
                "biom_name": biom_name,
            },
            "uuid": auth_uuid,
        }
        ...
        service = {
            "initial_key": service_public_key,
            "actor_type": "service",
            "uinfo": {
        ...
        try:
            ...
            app.db.execute("""TRUNCATE actor CASCADE""")
            subprocess.call(
                "psql postgresql://{USER}:{PASSWORD}@{HOST}:{PORT}/{NAME} < ~/{0}_standalone_dump.sql".format(
                    service_name, **db
                ),
                shell=True,
            ...
            print(
                "\033[91mError with getting default groups. Check that your service is registered on auth and"
