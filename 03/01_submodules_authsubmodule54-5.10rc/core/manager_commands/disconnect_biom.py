...
class DisconnectBiom(Command):
    ...
    option_list = (Option("--dump", "-d", dest="dump"),)

    def run(self, dump=None):

        if app.config.get("AUTH_STANDALONE") is False:
            print(
                "\033[91mYou can't use this command with AUTH_STANDALONE = False\033[0m"
            )
            return
        ...
        if dump:
            ...
            subprocess.call(
                "psql postgresql://{USER}:{PASSWORD}@{HOST}:{PORT}/{NAME} < {dump}".format(
                    **db, dump=dump
                ),
                shell=True,
            ...
            print(
                f"\033[92mSuccessfully disconnected from biom with restore db from dump.\033[0m"
            )
            return
        ...
        default = {
            "actor_type": "group",
            "uinfo": {"weight": 0, "group_name": "DEFAULT"},
            "uuid": uuid4(),
        }
        admin = {
            "actor_type": "group",
            "uinfo": {"weight": 4294967298, "group_name": "ADMIN"},
            "uuid": uuid4(),
        }
        ban = {
            "actor_type": "group",
            "uinfo": {"weight": 4294967299, "group_name": "BAN"},
            "uuid": uuid4(),
        }
