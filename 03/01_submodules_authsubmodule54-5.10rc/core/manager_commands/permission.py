...
from ..utils import (check_if_auth_service, delete_old_permactions,
                     get_auth_domain, get_language_header,
                     insert_or_update_default_permaction, json_dumps)
...
class CollectPerms(BaseCommand):
    def run(self):
        ...
        if not app.config.get("SERVICE_UUID"):
            ...
            result.update(permaction_message=permaction_message,)
    ...
    def execute_command(self):
        ...
        permaction_message = self.save_permissions(permaction_result)
    ...
    def collect_perms(self, actions_files: Tuple[str]) -> Tuple[List[Dict], List[Dict]]:
        ...
        for module_path in actions_files:
            ...
            [
                ...
                if "BasePermAction" in [base.__name__ for base in cls.__bases__]
            ]
    ...
    def save_permissions(self, permissions: List[Dict]) -> str:
        ...
        if permissions == []:
            ...
            signature = sign_data(
                ...
                json_dumps(permissions, sort_keys=True),
            )
            ...
            response = requests.post(
                urljoin(get_auth_domain(), "/service/permactions/default"),
                json=request_data,
                ...
                if content.get("error"):
                    result_message = "Response status - %s \nResponse content - %s" % (
                        ...
                        "\n".join(
                            [
                                f"{field}: {message}"
                                for field, message in content.get(
                                    "error_content", {}
                                ).items()
                            ]
                        ),
