    ...
    def post(self, **kwargs):
        ...
        if not check_if_auth_service():
            if not app.config.get('AUTH_STANDALONE'):
                ...
                response, status = self.complete_service_registration(data=data, sid=sid)
                    ...
                    if response.ok:
                        ...
                        if not exists:
                            ...
                            try:
                                ...
                                app.db.execute(query, values)
                                insert_or_update_actor_permaction(permissions=permactions)
                            except Exception as e:
                                print('Exception on creating user! %s' % e)
                                print_error_cli(message="Error with creating user. "
...
class AuthorizationView(MethodView):
    ...
    def collect_params(self):
        params = dict(
            ...
            ajax_login_url=urljoin(app.config.get("SERVICE_DOMAIN"), url_for('auth_submodule.auth_qr_login'))
        )
        return params
    ...
    def create_android_qr(self):
        android_qr = base64.b64encode(GetQRCodeView().generate_qr_image(
            ERP_APP_URL.get('android')).getvalue()).decode()
        return android_qr

    def create_ios_qr(self):
        ios_qr = base64.b64encode(GetQRCodeView().generate_qr_image(
    ...
    def create_registration_authorization_content(self, authentication_url, registration_url):
        ...
        registration_content["depended_services"] = \
            authentication_content.get("depended_services")
    ...
    def create_make_sso_login_function(
        ...
        **kwargs
        ...
        make_sso_login_base = \
            make_sso_login_base % (current_domain, "%s")
        ...
        **kwargs
        ...
        if not app.config.get('AUTH_STANDALONE'):
            ...
            check_cookie_base = check_cookie_base % \
                (temporary_session, cookie_names, current_name.capitalize(), urljoin(app.config['SERVICE_DOMAIN'], '/auth_sso_login/'))
        ...
        if data.get('qr_token'):
            ...
            for name, service_data in data.get("depended_services", {}).items():
                response_json = requests.post(
                    app.config.get('DEPENDED_SERVICES').get(name.lower()) + "/get_session/",
                    json=service_data
                ).json()
                if response_json:
            ...
            if temporary_sessions.get('temporary_session', None):
                ...
                if app.db.fetchone("""SELECT EXISTS(SELECT 1 FROM temporary_session WHERE temporary_session = %s)""",
                    ...
                    for name, temporary_session in temporary_sessions.items():
                        ...
                        response_json = requests.post(
                            app.config.get('DEPENDED_SERVICES').get(service_name) + "/get_session/",
