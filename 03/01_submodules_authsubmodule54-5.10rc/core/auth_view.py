...
from . import ERP_APP_URL, auth_submodule
...
from .utils import (actor_exists, apt54_expired, check_if_auth_service,
                    create_actor, create_new_salt, create_response_message,
                    create_session, create_temporary_session,
                    delete_temporary_session, generate_qr_token, get_apt54,
                    get_apt54_locally, get_auth_domain, get_default_user_group,
                    get_language_header, get_public_key, get_service_locale,
                    get_session_token_by_auxiliary, get_static_group,
                    get_user_salt, get_user_sid, hash_md5,
                    insert_or_update_actor_permaction, json_dumps,
                    print_error_cli, update_salt_data, update_user,
                    validate_email, verify_apt54)
...
class AuthSSOGenerationView(MethodView):
    ...
    def post(self):
        ...
        if not data or not signature:
            print_error_cli(
                message="Error with data. core.auth_view.AuthSSOView - POST.\n "
        ...
        if not apt54:
            print_error_cli(
                message="There is no APT54 token. core.auth_view.AuthSSOView - POST.\n data - %s"
                % data
            )
            response = create_response_message(
                message=_(
...
class GetQRCodeView(MethodView):
    ...
    def get(self, **kwargs):
        if request.args.get("qr_type", None) == "application":
            ...
            response = dict(qr_code=base64.b64encode(img_io.getvalue()).decode(),)
            return make_response(jsonify(response), 200)
        ...
        registration_url = (
            ...
            if data.get("registration_url")
                ...
                service_domain.get("service_domain"), url_for("auth_submodule.reg")
            )
        )
        apt54_url = urljoin(
            service_domain.get("service_domain"), url_for("auth_submodule.apt54")
        )
        authentication_url = (
            data.get("authentication_url")
            if data.get("authentication_url")
            else urljoin(
                service_domain.get("service_domain"), url_for("auth_submodule.auth")
            )
        )
        about_url = (
            data.get("about_url")
            if data.get("about_url")
                ...
                service_domain.get("service_domain"), url_for("auth_submodule.about")
            )
        )
        ...
        if qr_type == "registration":
            ...
            response.update({"depended_services": self.get_depended_qr_info()})
        return make_response(jsonify(response), 200)
...
class BaseAuth(MethodView):
    ...
    def check_if_from_client_service(data):
        ...
        if "service_uuid" in data:
            ...
            if not service_info:
                print_error_cli(
                    message="Unknown service. core.auth_view.BaseAuth - check_if_from_client_service.\n "
            ...
            if not verify_signature(
                ...
                print_error_cli(
                    message="Signature verification error. core.auth_view.BaseAuth - "
    ...
    def add_default_group(data):
        """
        Validate default group received from client service
        or adding auth default group.
        :param data: request data
        :return: uinfo or dict with error True and error_message.
        @subm_flow Validate default group received from client service or adding auth default group
        """
        ...
        if uinfo.get("groups", None):
            ...
            if len(groups_uuid) != len(groups):
                ...
                print_error_cli(
                    message="Unknown group. core.auth_view.BaseAuth - add_default_group.\n "
                    "invalid_groups - %s" % invalid_groups
                )
                response = create_response_message(
                    message=_(
                        "There is no such groups %(invalid_groups)s",
            ...
            if groups_uuid:
                ...
                if not admin_group:
                    print_error_cli(
                        message="There is no admin group. core.auth_view.BaseAuth - add_default_group.\n"
                    )
                    response = create_response_message(
                        message=_("Some error occurred with admin group."), error=True
                    )
                    return response

                if admin_group.get("uuid") in groups:
                    print_error_cli(
                        message="Default group is admin. core.auth_view.BaseAuth - add_default_group.\n "
                        "data - %s" % data
                    )
                    response = create_response_message(
                        message=_(
                            "This group can't be used by default " "for your service."
                        ),
                        error=True,
                    )
                    return response
    ...
    def upgrade_salt_for(salt, actor_uuid, qr_token, salt_for="authentication"):
        app.db.execute(
            """UPDATE salt_temp SET salt_for = %s WHERE salt = %s AND uuid = %s AND qr_token = %s""",
...
class RegistrationView(BaseAuth):
    ...
    def post(self, **kwargs):
        ...
        if not check_if_auth_service():
            if not app.config.get("AUTH_STANDALONE"):
                ...
                response, status = self.complete_service_registration(
                    data=data, sid=sid
                )

                return make_response(jsonify(response), status)
        response, status = self.complete_auth_registration(data=data, sid=sid)

        return make_response(jsonify(response), status)

    def complete_auth_registration(self, data, sid):
    ...
    def complete_service_registration(self, data, sid):
    ...
    def registration(self, data: dict):
        ...
        if uinfo.get("email", None):
            if app.db.fetchone(
                """SELECT EXISTS(SELECT 1 FROM actor WHERE uinfo ->> 'email' = %s)""",
                [uinfo.get("email")],
            ).get("exists"):
                print_error_cli(
        ...
        try:
            user = app.db.fetchone(
                """INSERT INTO actor(initial_key, uinfo) VALUES (%s, %s::jsonb) RETURNING *""",
                [pub_key, json_dumps(uinfo)],
            )
        except Exception as e:
            print(
                "Exception on creating user! RegistrationView - registration. \n Exception - %s"
                % e
            )
            user = None
        ...
        if result.get("from_service"):
            ...
            response = dict(user_uuid=user.get("uuid"))
    ...
    def registration_classic_user(self, data: dict):
        ...
        if uinfo:
            ...
            if "email" in uinfo:
                print_error_cli(
                    message="Email is in uinfo.core.auth_view.RegistrationView - registration_classic_user"
                )
                response = create_response_message(
                    message=_("Invalid parameter email in optional data."), error=True
                )
                return response, 400
        ...
        if data.get("identifier", None):
            invite_link_info = app.db.fetchone(
                """SELECT service_uuid, link_uuid FROM invite_link_temp 
            WHERE params->>'identifier' = %s""",
                [data.get("identifier")],
            )

            if invite_link_info:
                service_info = app.db.fetchone(
        ...
        uinfo.update(dict(email=email, password=password))

        query = (
            "INSERT INTO actor(uinfo, actor_type) VALUES (%s::jsonb, %s) RETURNING *"
        )
    ...
    def collect_data_for_auth(data, salt_for: str = None):
        ...
        if data.get("actor_type", None) != "classic_user":
            data["salt"] = get_user_salt(
                {"qr_token": data.get("qr_token")}, salt_for=salt_for
            )
...
class APT54View(MethodView):
    ...
    def post(self, **kwargs):
        ...
        if data.get("step", None) and data.get("step", None) == 1 and data.get("uuid"):
            salt = create_new_salt(
                {"uuid": data.get("uuid")}, salt_for="authentication"
            )
    ...
    def authentication(self, data: dict):
        ...
        if step:
            ...
            salt = get_user_salt(
                {"qr_token": qr_token, "uuid": uuid}, salt_for="authentication"
            )
...
class ClientAuthView(BaseAuth):
    ...
    def post(self, **kwargs):
            ...
            if not check_if_auth_service():
                if not app.config.get("AUTH_STANDALONE"):
                    ...
                    if response.ok:
                        ...
                        permactions = response_data.pop("permactions")
                        ...
                        if not exists:
                            ...
                            try:
                                ...
                                app.db.execute(query, values)
                                insert_or_update_actor_permaction(
                                    permissions=permactions
                                )
                            except Exception as e:
                                print("Exception on creating user! %s" % e)
                                print_error_cli(
                        ...
                        session_token = create_session(
                            apt54, depended_info=data.get("depended_services")
                        )
                        if isinstance(session_token, dict) and session_token.get(
                            "error"
                        ):
                        ...
                        if (
                            session_token
                            and app.config.get("SESSION_STORAGE", None) == "SESSION"
        ...
        if data.get("step", None) and data.get("step", None) == 1:
            apt54 = (
                data.get("apt54")
                if isinstance(data.get("apt54"), dict)
                else json.loads(data.get("apt54"))
            )
            if not isinstance(apt54.get("user_data"), dict) or not apt54.get(
                "user_data"
            ):
                # There is no user info in apt54
                print_error_cli(
                    message="Error with apt54 on step 1. "
                    "core.auth_view.ClientAuthView - POST.\n apt54 - %s" % apt54
                )
                response = create_response_message(
                    message=_(
            ...
            salt = create_new_salt(
                {"uuid": apt54["user_data"].get("uuid")}, salt_for="authentication"
            )

            if not salt:
                print_error_cli(
                    message="Error with creating salt. core.auth_view.ClientAuthView - POST.\n "
            ...
            if app.config.get("DEPENDED_SERVICES"):
                depended_services = {
                    name: data
                    for name, _ in app.config.get("DEPENDED_SERVICES").items()
                }
        ...
        if status == 200:
            ...
            if (
                app.config.get("SESSION_STORAGE", None) == "SESSION"
                or app.config.get("SESSION_STORAGE") is None
            ):
                session["session_token"] = response.get("session_token")
    ...
    def get_depended_services_info(self, authorized_info):
        """
        Get authentication information from depended services
        @subm_flow
        """
        depended_services_info = dict()
        for name, domain in app.config.get("DEPENDED_SERVICES").items():
            service_info = requests.post(
                urljoin(domain, "/auth/"), json=authorized_info.get(name)
            )
            depended_services_info.update({name: service_info.json()})
    ...
    def authorization(data: dict):
        ...
        if not isinstance(apt54.get("user_data"), dict) or not apt54.get("user_data"):
            # There is no user info in apt54
            print_error_cli(
                message="Error with apt54. core.auth_view.ClientAuthView - authorization.\n "
                "apt54 - %s" % apt54
            )
            response = create_response_message(
                message=_(
        ...
        if not salt:
            # There is no salt generated for that public key
            print_error_cli(
                message="Error with getting salt. core.auth_view.ClientAuthView - authorization.\n "
                "apt54 - %s" % apt54
            )
            response = create_response_message(
                message=_(
        ...
        if not initial_key and not secondary_keys:
            ...
            if not initial_key:
                print_error_cli(
                    message="Error with getting initial_key, even in apt54. "
        ...
        if verify_signature(initial_key, signed_salt, salt):
            ...
            if not verify_apt54(apt54):
                print_error_cli(
                    message="APT54 is invalid. core.auth_view.ClientAuthView - authorization.\n "
                    "apt54 - %s" % apt54
                )
                response = create_response_message(
                    message=_(
                        "Your authentication token is invalid. Please try again "
            ...
            if apt54_expired(apt54.get("expiration")):
                ...
                if status_code == 452:
                    print_error_cli(
                        message="Error with getting APT54. There is no such actor uuid -%s. "
                        "core.auth_view.ClientAuthView - authorization.\n "
                        "result - %s" % (uuid, apt54)
                    )
                    response = create_response_message(
                        message=_(
                            "There is no such actor. Please try again or contact the administrator."
                        ),
                        error=True,
                    )
                    return response, 400
                    ...
                    print_error_cli(
                        message="Error with getting APT54. Auth is unreachable. uuid - %s "
                        "core.auth_view.ClientAuthView - authorization.\n "
                        "result - %s" % (uuid, apt54)
                    )
                    response = create_response_message(
                        message=_(
                            "Auth service is unreachable. Please try again or "
                    ...
                    print_error_cli(
                        message="Error with getting APT54. APT54 expired. uuid - %s "
                        "core.auth_view.ClientAuthView - authorization.\n "
                        "result - %s" % (uuid, apt54)
                    )
                    response = create_response_message(
            ...
            response = create_session(
                apt54,
                auxiliary_token=data.get("qr_token"),
                depended_info=data.get("depended_services"),
            )
            if isinstance(response, dict) and response.get("error"):
                return response, 403
            ...
            if not session_token:
                print_error_cli(
                    message="Error with creating session_token. "
                    "core.auth_view.ClientAuthView - authorization.\n session_token- %s"
                    % response
                )
                response = create_response_message(
                    message=_(
                        "Some error occurred while creating session. "
                        "Please try again or contact the administrator."
                    ),
                    error=True,
                )
                ...
                status = 200
                response = dict(apt54=json_dumps(apt54), session_token=session_token)
            ...
            if secondary_keys:
                for public_key in secondary_keys:
                    ...
                    if verify_signature(public_key, signed_salt, salt):
                        ...
                        if not updated:
                            if app.config.get("AUTH_STANDALONE"):
                                updated_apt54, status_code = get_apt54_locally(
                                    uuid=uuid
                                )
                            else:
                                updated_apt54, status_code = get_apt54(uuid=uuid)
                        ...
                        if not session_token:
                            ...
                            if updated:
                                update_user(apt54)
                            permactions = app.db.fetchall(
                                """SELECT actor_uuid as actor_uuid,
                                    params as params,
                                    permaction_uuid as permaction_uuid,
                                    service_uuid as service_uuid,
                                    value as value
                                FROM actor_permaction
                                WHERE actor_uuid = %s AND service_uuid = %s;
                            """,
                                (uuid, data.get("service_uuid")),
                            )
                            status = 200
                            response = dict(
        ...
        print_error_cli(
            message="Signature verification failed with both keys - initial and secondary. "
            "core.auth_view.ClientAuthView - authorization.\n "
        )
        response = create_response_message(
            message=_("Signature verification failed."), error=True
    ...
    def authorization_classic_user(self, data: dict):
        ...
        :return: response, status: response - dictionary with session_token of error=True flag with error message,
         status - http code
        @subm_flow Authorization classic user
        """
        ...
        user = app.db.fetchone(
            """SELECT * FROM actor WHERE uinfo ->> 'email' = %s AND actor_type = 'classic_user'""",
            [email],
        )
        ...
        if not user:
            ...
            response = create_response_message(
                message=_("There is no actor with such email. Please check it."),
                error=True,
            )
            return response, 400
        ...
        if status_code == 452:
            print_error_cli(
                message="Error with getting APT54 for classic user. There is no such actor uuid -%s. "
                "core.auth_view.ClientAuthView - authorization.\n "
            ...
            print_error_cli(
                message="Error with getting APT54 for classic user. Auth is unreachable. uuid - %s "
                "core.auth_view.ClientAuthView - authorization.\n "
                "result - %s" % (user.get("uuid"), apt54)
            )
            response = create_response_message(
            ...
            print_error_cli(
                message="Error with getting APT54 for classic user. APT54 expired. uuid - %s "
                "core.auth_view.ClientAuthView - authorization.\n "
                "result - %s" % (user.get("uuid"), apt54)
            )
            response = create_response_message(
        ...
        permactions = app.db.fetchall(
            """SELECT actor_uuid as actor_uuid,
                    ...
                    permaction_uuid as permaction_uuid,
                    service_uuid as service_uuid,
                    value as value FROM actor_permaction
                WHERE actor_uuid = %s AND service_uuid = %s;
            """,
            (user.get("uuid"), data.get("service_uuid")),
        )
...
class SaveSession(MethodView):
    ...
    def post(self):
        ...
        response = dict(message=_("Session token successfully saved."))
        return make_response(jsonify(response), 200)
    ...
    def post(self):
        ...
        if temporary_session:
            if app.db.fetchone(
                ...
                session_token = get_session_token_by_auxiliary(temporary_session)
                if session_token:
                    app.db.execute(
                        """UPDATE service_session_token SET auxiliary_token = NULL WHERE auxiliary_token = %s""",
                        [temporary_session],
                    )
...
class AuthorizationView(MethodView):
    """
    @GET Submodule Biom mode. Get login template. If in SESSION TOKEN is SESSION, automatically adding js, css scripts from static folder.@
    """
    ...
    def get(self, **kwargs):
        ...
        return render_template("auth.html", scripts=scripts, **params)

    def collect_params(self):
        params = dict(
            is_auth_service=check_if_auth_service(),
            services=self.create_services(),
            ios_qr=self.create_ios_qr(),
            android_qr=self.create_android_qr(),
            current_language=self.define_language(),
            standalone=app.config.get("AUTH_STANDALONE"),
            sso_mode=app.config.get("SSO_MODE"),
            language_information=app.config.get("LANGUAGES_INFORMATION", []),
            ajax_login_url=urljoin(
                app.config.get("SERVICE_DOMAIN"),
                url_for("auth_submodule.auth_qr_login"),
            ),
        )
        return params
    ...
    def create_scripts(self, **kwargs):
        if app.config["SESSION_STORAGE"] == "SESSION":
            ...
            js_parts = self.load_parts(
                js_folder + "js_fragments.json", js_folder + "js_fragments/"
            )
            js_libs_names = ["qrLib.js", "x-notify.js"]

            locals().update(self.crate_locals(**kwargs))
            locals().update(js_parts)
            variables = self.clear_variables(wrong_name="self", variables=locals())
    ...
    def collect_creators(self):
        ...
        variable_creators = [self.create_authentication_qr_token]
        return part_creators, variable_creators

    def clear_variables(self, wrong_name, variables):
        variables = {
            name: value for name, value in variables.items() if name != wrong_name
        }
        return variables

    def create_android_qr(self):
        android_qr = base64.b64encode(
            GetQRCodeView().generate_qr_image(ERP_APP_URL.get("android")).getvalue()
        ).decode()
        return android_qr

    def create_ios_qr(self):
        ios_qr = base64.b64encode(
            GetQRCodeView().generate_qr_image(ERP_APP_URL.get("ios")).getvalue()
        ).decode()
        return ios_qr
    ...
    def create_services(self):
        ...
        if not app.config.get("AUTH_STANDALONE"):
            ...
            try:
                ...
                services = json.loads(response.content).get("services")
            except Exception as e:
                services = list()
        return services

    def create_content(self, data):
        response = GetQRCodeView().get(data=data)
    ...
    def crate_locals(self, **kwargs):
        service_domain = self.get_service_domain()
        registration_url, authentication_url = self.create_urls(
            service_domain=service_domain, **kwargs
        )
        is_auth_service = check_if_auth_service()
        standalone = app.config.get("AUTH_STANDALONE")
        sso_mode = app.config.get("SSO_MODE")
        ajax_login_url = urljoin(
            app.config.get("SERVICE_DOMAIN"), url_for("auth_submodule.auth_qr_login")
        )
        ...
        save_session_url = (
            kwargs.get("save_session_url")
            if kwargs and kwargs.get("save_session_url")
        ...
        base_error_message_js = _(
            "Some error has occurred, please contact the administrator"
        )
        if not app.config.get("AUTH_STANDALONE"):
            (
            ...
            registration_qr_token = dict(
                qr_token=registration_content.get("qr_token"), qr_type="registration",
            )
            authentication_qr_token = self.create_authentication_qr_token(
                authentication_content
            ).get("authentication_qr_token")

            get_qr_url = (
                kwargs.get("get_qr_url")
                if kwargs and kwargs.get("get_qr_url")
                else url_for("auth_submodule.qr-code")
            )
            get_qr_url = app.config.get("SERVICE_DOMAIN") + get_qr_url
            auth_sso_url = (
                kwargs.get("auth_sso_url")
                if kwargs and kwargs.get("auth_sso_url")
    ...
    def create_registration_authorization_content(
        self, authentication_url, registration_url
    ):
        ...
        authentication_data = dict(
            qr_type="authentication", authentication_url=authentication_url,
        )
    ...
    def create_make_sso_login_function(
        ...
        **kwargs
        ...
        make_sso_login_base = make_sso_login_base % (current_domain, "%s")
        for name, domain in depended_services.items():
            make_sso_login_base = (
                make_sso_login_base % make_sso_login_fetch_part % (domain, name, "%s")
            )
        make_sso_login_base = make_sso_login_base % make_sso_login_form_part
    ...
    def create_authorization_response_event(
        ...
        **kwargs
        ...
        if not app.config.get("AUTH_STANDALONE"):
            ...
            authorization_response_event = authorization_response_event % cookie
    ...
    def create_check_cookie_function(
        ...
        **kwargs
        ...
        if not app.config.get("AUTH_STANDALONE"):
            ...
            for name, _ in depended_services.items():
                cookie_names += (
                    "\n" + 'deleteCookie("temporary_session_' + name.lower() + '")'
                )
                temporary_session += (
                    ",\ntemporary_session_"
                    + name.lower()
                    + ':getCookie("temporary_session_'
                    + name.lower()
                    + '")'
                )
            check_cookie_base = check_cookie_base % (
                ...
                cookie_names,
                current_name.capitalize(),
                urljoin(app.config["SERVICE_DOMAIN"], "/auth_sso_login/"),
            )
            return check_cookie_base
        return ""

    @staticmethod
    def create_authentication_qr_token(authentication_content: dict = {}, **kwargs):
        ...
        if authentication_content.get("depended_services"):
            authentication_qr_token.update({"depended_services": {}})
    ...
    def create_update_qr_function(
        update_qr_function: str = "", get_qr_url: str = "", **kwargs
    ):
        if not app.config.get("AUTH_STANDALONE"):
            update_qr_function = update_qr_function % (
                get_qr_url,
                get_qr_url,
                kwargs["registration_qr_token"],
                kwargs["authentication_qr_token"],
                kwargs["authentication_content"],
                kwargs["registration_content"],
            )
    ...
    def create_after_save_session_function(
        after_save_session_function: str, reload_part: str, **kwargs
    ):
...
class AuthQRCodeAuthorizationView(MethodView):
    ...
    def post(self):
        form = list(request.form.to_dict())
        data = json.loads(form[0])
            ...
            if temporary_sessions.get("temporary_session", None):
                ...
                if app.db.fetchone(
                    ...
                    for name, temporary_session in temporary_sessions.items():
                        ...
                        response_json = requests.post(
                            app.config.get("DEPENDED_SERVICES").get(service_name)
                            + "/get_session/",
                            json={"temporary_session": temporary_session},
                        ).json()
                        if response_json:
                            session_token[
                                service_name + "_session_token"
