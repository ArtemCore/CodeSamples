...
class Migration(base.BaseMigration):
    ...
    forwards_query = """
        ...
        -- Main function that updates actors and create new one if one of actors group exists on service or actor type is 'service'.
          ...
          BEGIN
            FOR user_uuid, user_root_perms_signature, user_initial_key, user_secondary_keys, user_uinfo, user_actor_type in SELECT uuid, root_perms_signature, initial_key, secondary_keys, uinfo, actor_type FROM get_received_actors(actors) LOOP
              ...
              ELSE
                ...
                IF user_actor_type = 'service' OR EXISTS(SELECT * FROM actor WHERE actor_type = 'group' AND uuid::text IN (SELECT jsonb_array_elements_text(user_uinfo->'groups'))) THEN
