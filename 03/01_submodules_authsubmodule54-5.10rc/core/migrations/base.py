...
from psycopg2.errors import (DuplicateColumn, DuplicateObject, DuplicateTable,
                             UndefinedColumn, UndefinedTable, UniqueViolation)
...
class BaseMigration:
    ...
    def forwards(self):
        if not self.forwards_query:
            raise ValueError("forwards_query can not be None or empty")
    ...
    def backwards(self):
        if not self.backwards_query:
            raise ValueError("backwards_query can not be None or empty")
