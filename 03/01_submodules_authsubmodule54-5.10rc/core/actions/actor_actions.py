...
from ..utils import (create_response_message, get_current_actor, hash_md5,
                     is_valid_uuid, json_dumps, validate_email)
...
class ActorAction(ABC):
    ...
    def create_response(self):
        if self.status_code == 200:
            self.response = dict(message=_(self.message))
        else:
            self.response = create_response_message(message=_(self.message), error=True)
...
class CreateActorAction(ActorAction):
    ...
    def initialize(self, data):
        ...
        self.actor["uuid"] = (
            self.actor.get("uuid") if self.actor.get("uuid") else uuid4()
        )
    ...
    def create_query(self):
        ...
        self.values.update({"actor": json_dumps(self.actor)})

    def handle_classic_user_data(self):
        if not self.uinfo.get("email") or not self.uinfo.get("password"):
    ...
    def handle_users_groups(self):
        if not self.uinfo.get("groups"):
            self.uinfo = RegistrationView.add_default_group(self.actor)
        else:
            try:
...
class UpdateActorAction(ActorAction):
    ...
    def prepare_values(self):
        ...
        if not self.actor_type:
            self.actor_type = app.db.fetchone(
                "SELECT actor_type FROM actor WHERE uuid = %s", (self.uuid,)
            ).get("actor_type")
    ...
    def handle_unique_weight(self):
        actor_count = app.db.fetchone(
            """SELECT COUNT(*) FROM actor WHERE actor_type = 'group'
    ...
    def handle_unique_user_email(self):
    ...
    def create_group_appends_list(self):
        self.appends_list += [
            sql.Placeholder("null_groups"),
            sql.Placeholder("group_uuid"),
            sql.Placeholder("group_jsonb_key"),
            sql.Placeholder("group_uuid"),
            sql.Placeholder("group_jsonb_key"),
            sql.Placeholder("group_uuid"),
    ...
    def create_classic_user_appends_list(self):
        self.appends_list += [sql.Placeholder("uinfo"), sql.Placeholder("uuid")]

    def create_classic_user_values(self):
        self.values.update(
            {"uinfo": json_dumps(self.uinfo), "uuid": self.uuid,}
        )
...
class DeleteActorAction(ActorAction):
    ...
    def create_appends_list(self):
        self.appends_list += [sql.Placeholder("uuid")]

    def create_values(self):
        self.values.update({"uuid": self.uuid})
    ...
    def handle_admin(self):
        for actor in self.actors:
            if actor.is_admin and (
                not self.current_actor.is_root or not self.current_actor.is_admin
            ):
                self.message = (
                    f"You can't delete admin.\n UUID - {self.actor.uuid},"
    ...
    def create_plural_appends_list(self):
        self.appends_list += [sql.Placeholder("uuid_list")]

    def create_plural_values(self):
        self.values.update({"uuid_list": self.uuid_list})
