...
from .manager_commands.migration import MigrateBackwards, MigrateForwards
from .manager_commands.permission import CollectPerms
from .manager_commands.root_user import CreateRootUser
from .manager_commands.sync_groups import SyncGroups
...
def init_manager(application, database=None):
    if not hasattr(application, "db"):
        if not database:
            print(
                "There is no database manager in main app or database credentials for registering this manager. "
    ...
    manager.add_command("sync_groups", SyncGroups())
    return manager
